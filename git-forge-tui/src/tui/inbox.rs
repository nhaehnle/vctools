...
impl Inbox {
    ...
    pub fn build(self, builder: &mut Builder, connections: &mut github::connections::Connections) -> InboxResult {
        ...
        for (host, client) in connections.all_clients() {
            ...
            Ok(notifications) => {
                for notification in notifications.into_iter() {
                    let id = table_builder.add(top_level, notification.id.clone())
