...
fn do_main() -> Result<()> {
    ...
    (old @ RevSpec::Range(_, _), new @ RevSpec::Range(_, _)) => {
        if args.combined {
            ...
            let match_lines: Vec<_> = range_diff.matches.iter().map(|rd_match| {
                let (old_idx, old_hash) = rd_match.old.as_ref()
                    .map(|(idx, hash)| (Some(format!("{idx}")), Some(format!("{hash}"))))
                    .unwrap_or((None, None));
                let (new_idx, new_hash) = rd_match.new.as_ref()
                    .map(|(idx, hash)| (Some(format!("{idx}")), Some(format!("{hash}"))))
                    .unwrap_or((None, None));
            ...
            fn max_len<'a, I: Iterator<Item = &'a Option<String>>>(iter: I) -> usize {
                iter.filter_map(|it| it.as_ref().map(|s| s.len())).max().unwrap_or(1)
            }
